...
public class Entities {
    ...
    private static var multipoints = Dictionary<String, String>() // name -> multiple character references
    ...
    public struct EscapeMode: Equatable {
        ...
        private func indexes(of codepoint: Int, in keys: [Int]) -> [Int] {
            var indexes = [Int]()
        ...
        public static func ==(lhs: EscapeMode, rhs: EscapeMode) -> Bool {
    ...
    public static func isNamedEntity(_ name: String) -> Bool {
        return EscapeMode.full.codepoint(forName: name) != nil
    }
    ...
    public static func isBaseNamedEntity(_ name: String) -> Bool {
        return EscapeMode.base.codepoint(forName: name) != nil
    }
    ...
    public static func escape(string: String, outputSettings: OutputSettings) -> String {
        ...
        escape(accum: accum, string: string, outputSettings: outputSettings, inAttribute: false, normalizeWhite: false, stripLeadingWhite: false)
        return accum.stringValue
    }
    
    static func escape(accum: StringBuilder, string: String, outputSettings: OutputSettings,
                       inAttribute: Bool, normalizeWhite: Bool, stripLeadingWhite: Bool) {
        var lastWasWhite = false
        var reachedNonWhite = false
        let escapeMode: EscapeMode = outputSettings.escapeMode
        let encoder: String.Encoding = outputSettings.charset
        ...
        for codePoint in string.unicodeScalars {
            ...
            if (codePoint.value < Character.MIN_SUPPLEMENTARY_CODE_POINT) {
                ...
                case "\"":
                    ...
                    break
                default:
                    if (canEncode(c, fallback: encoder)) {
                        accum.append(c)
                    } else {
    ...
    private static func appendEncoded(accum: StringBuilder, escapeMode: EscapeMode, codePoint: UnicodeScalar) {
        ...
        if (name != nil) {
            ...
            accum.append("&#x").append(String.toHexString(int: Int(codePoint.value))).append(";")
        }
    }
    
    public static func unescape(_ string: String) -> String {
        return unescape(string: string, strict: false)
    }
     ...
     */
    public static func unescape(string: String, strict: Bool) -> String {
        return Parser.unescape(entities: string, inAttributes: strict)
    }
     ...
     * Jsoup: 167, 2
     */
    private static func canEncode(_ c: UnicodeScalar, fallback: String.Encoding) -> Bool {
