...
class CleanerTest: XCTestCase {
    ...
    func testSimpleBehavior() {
        ...
        var cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.simpleText)
        
        XCTAssertEqual("Hello <b>there</b>!", cleanHTML.strippedNewLines)
        
        html = "Hello <b>there</b>!"
        cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.simpleText)
    ...
    func testBasicBehavior() {
        ...
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basic)
    ...
    func testBasicWithImages() {
        ...
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basicWithImages)
    ...
    func testRelaxed() {
        ...
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.relaxed)
    ...
    func testRemoveTags() {
        ...
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basic.remove(tags: "a"))
    ...
    func testRemoveAttrs() {
        ...
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basic.remove(from: "blockquote", attributes: "cite"))
    ...
    func testRemoveEnforcedAttrs() {
        let html = "<div><p><A HREF='HTTP://nice.com'>Nice</a></p><blockquote>Hello</blockquote>"
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basic.remove(from: "a", attrEnforced: "rel"))
    ...
    func testRemoveProtocols() {
        ...
        let cleanHTML = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basic.remove(from: "a", attr: "href", protocols: "ftp", "mailto"))
    ...
    func testCleanAnchorProtocol() {
        ...
        var cleanHTML = SwiftySoup.clean(bodyHtml: validAnchor, whitelist: Whitelist.relaxed)
        XCTAssertEqual("<a>Valid anchor</a>", cleanHTML)
        
        cleanHTML = SwiftySoup.clean(bodyHtml: invalidAnchor, whitelist: Whitelist.relaxed)
        ...
        cleanHTML = SwiftySoup.clean(bodyHtml: validAnchor, whitelist: relaxedWithAnchor)
        XCTAssertEqual(validAnchor, cleanHTML)
        
        // An invalid anchor is never valid.
        cleanHTML = SwiftySoup.clean(bodyHtml: invalidAnchor, whitelist: relaxedWithAnchor)
        XCTAssertEqual("<a>Invalid anchor</a>", cleanHTML)
    }
    ...
    func testIsValidBodyHTML() {
        ...
        let nok7 = "</div>What"
        XCTAssert(SwiftySoup.isValid(bodyHtml: ok, whitelist: Whitelist.basic))
        XCTAssert(SwiftySoup.isValid(bodyHtml: ok1, whitelist:  Whitelist.basic));
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok1, whitelist: Whitelist.basic))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok2, whitelist: Whitelist.basic))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok3, whitelist: Whitelist.basic))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok4, whitelist: Whitelist.basic))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok5, whitelist: Whitelist.basic))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok6, whitelist: Whitelist.basic))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: ok, whitelist: Whitelist.none))
        XCTAssertFalse(SwiftySoup.isValid(bodyHtml: nok7, whitelist: Whitelist.basic))
    }
    ...
    func testIsValidDocument() {
        ...
        let okDoc = SwiftySoup.parse(html: ok)
        XCTAssert(cleaner.isValid(document: okDoc))
        XCTAssertFalse(cleaner.isValid(document: SwiftySoup.parse(html: nok)))
    ...
    func testResolvesRelativeLinks() {
        ...
        let clean = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basicWithImages, baseUri: "http://example.com/")
    ...
    func testPreservesRelativeLinksIfConfigured() {
        ...
        let clean = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basicWithImages.preserveRelativeLinks(true), baseUri: "http://example.com/")
        XCTAssertEqual("<a href=\"/foo\" rel=\"nofollow\">Link</a>\n<img src=\"/bar\"> \n<img>", clean)
    }
    
    func testDropsUnresolvableRelativeLinks() {
        let html = "<a href='/foo'>Link</a>";
        let clean = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basic)
        XCTAssertEqual("<a rel=\"nofollow\">Link</a>", clean)
    }
    
    func testHandlesCustomProtocols() {
        let html = "<img src='cid:12345' /> <img src='data:gzzt' />"
        let dropped = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basicWithImages)
        XCTAssertEqual("<img> \n<img>", dropped)
        
        let preserved = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.basicWithImages.add(to: "img", attr: "src", protocols: "cid", "data"))
    ...
    func testHandlesAllPseudoTag() {
        ...
        let clean = SwiftySoup.clean(bodyHtml: html, whitelist: whitelist)
    ...
    func testSupplyOutputSettings() {
        ...
        let customOut = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.relaxed, outputSettings: os, baseUri: "http://foo.com/")
        let defaultOut = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.relaxed, baseUri: "http://foo.com/")
        ...
        os.charset = .ascii
        os.escapeMode = .base
        let customOut2 = SwiftySoup.clean(bodyHtml: html, whitelist: Whitelist.relaxed, outputSettings: os, baseUri: "http://foo.com/")
    ...
    func testHandlesFramesets() {
        ...
        let clean = SwiftySoup.clean(bodyHtml: dirty, whitelist: Whitelist.basic)
        XCTAssertEqual("", clean) // nothing good can come out of that
        
        let dirtyDoc = SwiftySoup.parse(html: dirty)
    ...
    func testCleansInternationalText() {
        XCTAssertEqual("привет", SwiftySoup.clean(bodyHtml: "привет", whitelist: Whitelist.none))
    }
    
    func testScriptTagInWhiteList() {
        let whitelist = Whitelist.relaxed
        whitelist.add(tags: "script")
        XCTAssert(SwiftySoup.isValid(bodyHtml: "Hello<script>alert('Doh')</script>World !", whitelist: whitelist))
    }
