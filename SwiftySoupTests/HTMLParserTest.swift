...
class HTMLParserTest: XCTestCase {
    ...
    func testParsesQuiteRoughAttributes() {
        ...
        var doc = SwiftySoup.parse(html: html)
        XCTAssertEqual("<p =a>One<a <p>Something</a></p>\n" +
            "<a <p>Else</a>", doc.body?.html)
        
        doc = SwiftySoup.parse(html: "<p .....>")
    ...
    func testDropsUnterminatedAttribute() {
        ...
        let h1 = "<p id=\"foo"
        let doc = SwiftySoup.parse(html: h1)
        XCTAssertEqual("", doc.text)
    }
    
    func testParsesUnterminatedTextarea() {
        // don't parse right to end, but break on <p>
        let doc = SwiftySoup.parse(html: "<body><p><textarea>one<p>two")
    ...
    func testParsesUnterminatedOption() {
        ...
        let doc = SwiftySoup.parse(html: "<body><p><select><option>One<option>Two</p><p>Three</p>")
    ...
    func testTestSpaceAfterTag() {
        let doc = SwiftySoup.parse(html: "<div > <a name=\"top\"></a ><p id=1 >Hello</p></div>")
    ...
    func testHandlesTextAfterData() {
        ...
        let doc = SwiftySoup.parse(html: h)
    ...
    func testHandlesTextArea() {
        let doc = SwiftySoup.parse(html: "<textarea>Hello</textarea>")
    ...
    func testPreservesSpaceInTextArea() {
        ...
        let doc = SwiftySoup.parse(html: "<textarea>\n\tOne\n\tTwo\n\tThree\n</textarea>")
    ...
    func testPreservesSpaceInScript() {
        ...
        let doc = SwiftySoup.parse(html: "<script>\nOne\n\tTwo\n\tThree\n</script>")
        let expect = "\nOne\n\tTwo\n\tThree\n"
    ...
    func testDoesNotCreateImplicitLists() {
        ...
        let doc2 = SwiftySoup.parse(html: h2)
    ...
    func testHandlesNestedImplicitTable() {
        let doc = SwiftySoup.parse(html: "<table><td>1</td></tr> <td>2</td></tr> <td> <table><td>3</td> <td>4</td></table> <tr><td>5</table>")
    ...
    func testHandlesWhatWgExpensesTableExample() {
        ...
        let doc = SwiftySoup.parse(html: "<table> <colgroup> <col> <colgroup> <col> <col> <col> <thead> <tr> <th> <th>2008 <th>2007 <th>2006 <tbody> <tr> <th scope=rowgroup> Research and development <td> $ 1,109 <td> $ 782 <td> $ 712 <tr> <th scope=row> Percentage of net sales <td> 3.4% <td> 3.3% <td> 3.7% <tbody> <tr> <th scope=rowgroup> Selling, general, and administrative <td> $ 3,761 <td> $ 2,963 <td> $ 2,433 <tr> <th scope=row> Percentage of net sales <td> 11.6% <td> 12.3% <td> 12.6% </table>")
    ...
    func testHandlesTbodyTable() {
        let doc = SwiftySoup.parse(html: "<html><head></head><body><table><tbody><tr><td>aaa</td><td>bbb</td></tr></tbody></table></body></html>")
    ...
    func testHandlesImplicitCaptionClose() {
        let doc = SwiftySoup.parse(html: "<table><caption>A caption<td>One<td>Two")
    ...
    func testNoTableDirectInTable() {
        let doc = SwiftySoup.parse(html: "<table> <td>One <td><table><td>Two</table> <table><td>Three")
    ...
    func testIgnoresDupeEndTrTag() {
        let doc = SwiftySoup.parse(html: "<table><tr><td>One</td><td><table><tr><td>Two</td></tr></tr></table></td><td>Three</td></tr></table>"); // two </tr></tr>, must ignore or will close table
    ...
    func testHandlesBaseTags() {
        ...
        let doc = SwiftySoup.parse(html: h, baseUri: "http://foo/")
    ...
    func testHandlesProtocolRelativeUrl() {
        ...
        let doc = SwiftySoup.parse(html: html, baseUri: base)
    ...
    func testHandlesUnknownInlineTags() {
        ...
        let doc = SwiftySoup.parse(bodyFragment: h)
    ...
    func testParsesBodyFragment() {
        ...
        let doc = SwiftySoup.parse(bodyFragment: h, baseUri: "http://example.com")
    ...
    func testHandlesBaseWithoutHref() {
        ...
        let doc = SwiftySoup.parse(html: h, baseUri: "http://example.com/")
        let a = doc.select("a")[0]
    ...
    func testTestHgroup() {
        ...
        let doc = SwiftySoup.parse(html: "<h1>Hello <h2>There <hgroup><h1>Another<h2>headline</hgroup> <hgroup><h1>More</h1><p>stuff</p></hgroup>")
    ...
    func testTestRelaxedTags() {
        let doc = SwiftySoup.parse(html: "<abc_def id=1>Hello</abc_def> <abc-def>There</abc-def>")
    ...
    func testTestHeaderContents() {
        ...
        let doc = SwiftySoup.parse(html: "<h1>Hello <div>There</div> now</h1> <h2>More <h3>Content</h3></h2>")
    ...
    func testTestSpanContents() {
        ...
        let doc = SwiftySoup.parse(html: "<span>Hello <div>there</div> <span>now</span></span>")
    ...
    func testTestNoImagesInNoScriptInHead() {
        ...
        let doc = SwiftySoup.parse(html: "<html><head><noscript><img src='foo'></noscript></head><body><p>Hello</p></body></html>")
    ...
    func testTestAFlowContents() {
        ...
        let doc = SwiftySoup.parse(html: "<a>Hello <div>there</div> <span>now</span></a>")
    ...
    func testTestFontFlowContents() {
        ...
        let doc = SwiftySoup.parse(html: "<font>Hello <div>there</div> <span>now</span></font>")
    ...
    func testHandlesUnclosedTitleAtEof() {
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<title>Data").title)
        XCTAssertEqual("Data<", SwiftySoup.parse(html: "<title>Data<").title)
        XCTAssertEqual("Data</", SwiftySoup.parse(html: "<title>Data</").title)
        XCTAssertEqual("Data</t", SwiftySoup.parse(html: "<title>Data</t").title)
        XCTAssertEqual("Data</ti", SwiftySoup.parse(html: "<title>Data</ti").title)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<title>Data</title>").title)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<title>Data</title >").title)
    }
    
    func testHandlesUnclosedTitle() {
        let one = SwiftySoup.parse(html: "<title>One <b>Two <b>Three</TITLE><p>Test</p>"); // has title, so <b> is plain text
        XCTAssertEqual("One <b>Two <b>Three", one.title)
        XCTAssertEqual("Test", one.select("p").first?.text)
        
        let two = SwiftySoup.parse(html: "<title>One<b>Two <p>Test</p>"); // no title, so <b> causes </title> breakout
        XCTAssertEqual("One", two.title)
        XCTAssertEqual("<b>Two <p>Test</p></b>", two.body?.html)
    }
    
    func testHandlesUnclosedScriptAtEof() {
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data").select("script").first?.data)
        XCTAssertEqual("Data<", SwiftySoup.parse(html: "<script>Data<").select("script").first?.data)
        XCTAssertEqual("Data</sc", SwiftySoup.parse(html: "<script>Data</sc").select("script").first?.data)
        XCTAssertEqual("Data</-sc", SwiftySoup.parse(html: "<script>Data</-sc").select("script").first?.data)
        XCTAssertEqual("Data</sc-", SwiftySoup.parse(html: "<script>Data</sc-").select("script").first?.data)
        XCTAssertEqual("Data</sc--", SwiftySoup.parse(html: "<script>Data</sc--").select("script").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data</script>").select("script").first?.data)
        XCTAssertEqual("Data</script", SwiftySoup.parse(html: "<script>Data</script").select("script").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data</script ").select("script").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data</script n").select("script").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data</script n=").select("script").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data</script n=\"").select("script").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<script>Data</script n=\"p").select("script").first?.data)
    }
    
    func testHandlesUnclosedRawtextAtEof() {
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<style>Data").select("style").first?.data)
        XCTAssertEqual("Data</st", SwiftySoup.parse(html: "<style>Data</st").select("style").first?.data)
        XCTAssertEqual("Data", SwiftySoup.parse(html: "<style>Data</style>").select("style").first?.data)
        XCTAssertEqual("Data</style", SwiftySoup.parse(html: "<style>Data</style").select("style").first?.data)
        XCTAssertEqual("Data</-style", SwiftySoup.parse(html: "<style>Data</-style").select("style").first?.data)
        XCTAssertEqual("Data</style-", SwiftySoup.parse(html: "<style>Data</style-").select("style").first?.data)
        XCTAssertEqual("Data</style--", SwiftySoup.parse(html: "<style>Data</style--").select("style").first?.data)
    }
    ...
    func testNoImplicitFormForTextAreas() {
        ...
        let doc = SwiftySoup.parse(html: "<textarea>One</textarea>")
        XCTAssertEqual("<textarea>One</textarea>", doc.body?.html)
    }
    
    func testHandlesEscapedScript() {
        let doc = SwiftySoup.parse(html: "<script><!-- one <script>Blah</script> --></script>")
        XCTAssertEqual("<!-- one <script>Blah</script> -->", doc.select("script").first?.data)
    }
    
    func testHandles0CharacterAsText() {
        let doc = SwiftySoup.parse(html: "0<p>0</p>")
        XCTAssertEqual("0\n<p>0</p>", doc.body?.html)
    }
    
    func testHandlesNullInData() {
        let doc = SwiftySoup.parse(html: "<p id=\u{0000}>Blah \u{0000}</p>")
    ...
    func testTracksErrorsWhenRequested() {
        ...
        let doc = SwiftySoup .parse(html: html, baseUri: "http://example.com", parser: parser)
    ...
    func testHandlesCommentsInTable() {
        ...
        let node = SwiftySoup.parse(bodyFragment: html)
    ...
    /*func testInvalidTableContents() {
        ...
        let doc = SwiftySoup.parse(html: in, "UTF-8")
    ...
    func testTestNormalisesIsIndex() {
        let doc = SwiftySoup.parse(html: "<body><isindex action='/submit'></body>")
    ...
    func testTestReinsertionModeForThCelss() {
        ...
        let doc = SwiftySoup.parse(html: body)
    ...
    func testTestFragment() {
        ...
        let body = SwiftySoup.parse(bodyFragment: html)
    ...
    func testHandlesControlCodeInAttributeName() {
        let doc = SwiftySoup.parse(html: "<p><a \06=foo>One</a><a/\06=bar><a foo\06=bar>Two</a></p>")
