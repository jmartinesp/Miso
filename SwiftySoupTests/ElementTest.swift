...
class ElementTest: XCTestCase {
    ...
    func testGetElementsByTagName() {
        let document = SwiftySoup.parse(html: ElementTest.reference)
    ...
    func testGetNamespacedElementsByTag() {
        let document = SwiftySoup.parse(html: "<div><abc:def id=1>Hello</abc:def></div>")
    ...
    func testGetElementById() {
        ...
        let document2 = SwiftySoup.parse(html: "<div id=1><div id=2><p>Hello <span id=2>world!</span></p></div></div>")
    ...
    func testGetChildText() {
        let document = SwiftySoup.parse(html: "<p>Hello <b>there</b> now")
    ...
    func testBrHasSpace() {
        let doc = SwiftySoup.parse(html: "<p>Hello<br>there</p>")
        XCTAssertEqual("Hello there", doc.text);
        XCTAssertEqual("Hello there", doc.select("p").first?.ownText)
        
        let doc2 = SwiftySoup.parse(html: "<p>Hello <br> there</p>")
        XCTAssertEqual("Hello there", doc2.text)
    }
    
    func testGetSiblings() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello<p id=1>there<p>this<p>is<p>an<p id=last>element</div>")
    ...
    func testGetSiblingsWithDuplicateContent() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello<p id=1>there<p>this<p>this<p>is<p>an<p id=last>element</div>")
        let p = doc.element(byId: "1")
        XCTAssertEqual("there", p?.text)
        XCTAssertEqual("Hello", p?.previousSiblingElement?.text)
        XCTAssertEqual("this", p?.nextSiblingElement?.text)
    ...
    func testGetParents() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello <span>there</span></div>")
    ...
    func testElementSiblingIndex() {
        let doc = SwiftySoup.parse(html: "<div><p>One</p>...<p>Two</p>...<p>Three</p>")
        let ps = doc.select("p")
    ...
    func testElementSiblingIndexSameContent() {
        let doc = SwiftySoup.parse(html: "<div><p>One</p>...<p>One</p>...<p>One</p>")
        let ps = doc.select("p")
        XCTAssert(0 == ps[0].elementSiblingIndex)
        XCTAssert(1 == ps[1].elementSiblingIndex)
        XCTAssert(2 == ps[2].elementSiblingIndex)
    ...
    func testGetElementsWithClass() {
        let doc = SwiftySoup.parse(html: "<div class='mellow yellow'><span class=mellow>Hello <b class='yellow'>Yellow!</b></span><p>Empty</p></div>")
    ...
    func testGetElementsWithAttribute() {
        let doc = SwiftySoup.parse(html: "<div style='bold'><p title=qux><p><b style></b></p></div>")
    ...
    func testGetElementsWithAttributeDash() {
        let doc = SwiftySoup.parse(html: "<meta http-equiv=content-type value=utf8 id=1> <meta name=foo content=bar id=2> <div http-equiv=content-type value=utf8 id=3>")
    ...
    func testGetElementsWithAttributeValue() {
        let doc = SwiftySoup.parse(html: "<div style='bold'><p><p><b style></b></p></div>")
    ...
    func testClassDomMethods() {
        let doc = SwiftySoup.parse(html: "<div><span class=' mellow yellow '>Hello <b>Yellow</b></span></div>")
    ...
    func testClassUpdates() {
        let doc = SwiftySoup.parse(html: "<div class='mellow yellow'></div>")
    ...
    func testOuterHtml() {
        let doc = SwiftySoup.parse(html: "<div title='Tags &amp;c.'><img src=foo.png><p><!-- comment -->Hello<p>there")
    ...
    func testInnerHtml() {
        let doc = SwiftySoup.parse(html: "<div>\n <p>Hello</p> </div>")
        XCTAssertEqual("<p>Hello</p>", doc.elements(byTag: "div")[0].html)
    }
    
    func testFormatHtml() {
        let doc = SwiftySoup.parse(html: "<title>Format test</title><div><p>Hello <span>jsoup <span>users</span></span></p><p>Good.</p></div>")
    ...
    func testSetIndent() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello\nthere</p></div>")
    ...
    func testEmptyElementFormatHtml() {
        ...
        let doc = SwiftySoup.parse(html: "<section><div></div></section>")
    ...
    func testNoIndentOnScriptAndStyle() {
        ...
        let doc = SwiftySoup.parse(html: "<script>one\ntwo</script>\n<style>three\nfour</style>")
        XCTAssertEqual("<script>one\ntwo</script> \n<style>three\nfour</style>", doc.head?.html)
    }
    
    func testContainerOutput() {
        let doc = SwiftySoup.parse(html: "<title>Hello there</title> <div><p>Hello</p><p>there</p></div> <div>Another</div>")
    ...
    func testAddNewElement() {
        let doc = SwiftySoup.parse(html: "<div id=1><p>Hello</p></div>")
        let div = doc.element(byId: "1")
    ...
    func testAppendRowToTable() {
        let doc = SwiftySoup.parse(html: "<table><tr><td>1</td></tr></table>")
    ...
    func testSetHtmlTitle() {
        let doc = SwiftySoup.parse(html: "<html><head id=2><title id=1></title></head></html>")
    ...
    func testWrap() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p><p>There</p></div>")
    ...
    func testWrapWithRemainder() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p></div>")
        let p = doc.select("p").first
    ...
    func testHasText() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p><p></p></div>")
        let div = doc.select("div").first
        let ps = doc.select("p")
    ...
    func testDataSet() {
        let doc = SwiftySoup.parse(html: "<div id=1 data-name=jsoup class=new data-package=jar>Hello</div><p id=2>Hello</p>")
        let div = doc.select("div").first
    ...
    func parentlessToString() {
        let doc = SwiftySoup.parse(html: "<img src='foo'>")
    ...
    /*func testClone() {
        let doc = SwiftySoup.parse(html: "<div><p>One<p><span>Two</div>")
    ...
    func testClonesClassnames() {
        let doc = SwiftySoup.parse(html: "<div class='one two'></div>")
        let div = doc.select("div").first
    ...
    func testTagNameSet() {
        let doc = SwiftySoup.parse(html: "<div><i>Hello</i>")
    ...
    func testHtmlContainsOuter() {
        let doc = SwiftySoup.parse(html: "<title>Check</title> <div>Hello there</div>")
        doc.outputSettings.indentAmount = 0
    ...
    func testGetTextNodes() {
        let doc = SwiftySoup.parse(html: "<p>One <span>Two</span> Three <br> Four</p>")
    ...
    func testGetDataNodes() {
        let doc = SwiftySoup.parse(html: "<script>One Two</script> <style>Three Four</style> <p>Fix Six</p>")
    ...
    func testElementIsNotASiblingOfItself() {
        let doc = SwiftySoup.parse(html: "<div><p>One<p>Two<p>Three</div>")
    ...
    func testMoveByAppend() {
        ...
        let doc = SwiftySoup.parse(html: "<div id=1>Text <p>One</p> Text <p>Two</p></div><div id=2></div>")
    ...
    func testInsertChildrenAtPosition() {
        let doc = SwiftySoup.parse(html: "<div id=1>Text1 <p>One</p> Text2 <p>Two</p></div><div id=2>Text3 <p>Three</p></div>")
        let div1 = doc.select("div")[0]
    ...
    func testCssPath() {
        let doc = SwiftySoup.parse(html: "<div id=\"id1\">A</div><div>B</div><div class=\"c1 c2\">C</div>")
    ...
    func testClassNames() {
        let doc = SwiftySoup.parse(html: "<div class=\"c1 c2\">C</div>")
    ...
    func testHashAndEqualsAndValue() {
        ...
        let doc = SwiftySoup.parse(html: doc1)
    ...
    func testRelativeUrls() {
        ...
        let doc = SwiftySoup.parse(html: html, baseUri: "http://example.com/bar/")
