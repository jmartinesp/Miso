...
class ElementsTest: XCTestCase {
    ...
    func testHasAttr() {
        let doc = SwiftySoup.parse(html: "<p title=foo><p title=bar><p class=foo><p class=bar>")
        let ps = doc.select("p")
    ...
    func testHasAbsAttr() {
        let doc = SwiftySoup.parse(html: "<a id=1 href='/foo'>One</a> <a id=2 href='https://jsoup.org'>Two</a>")
    ...
    func testClasses() {
        let doc = SwiftySoup.parse(html: "<div><p class='mellow yellow'></p><p class='red green'></p>")
    ...
    func testHasClassCaseInsensitive() {
        let els = SwiftySoup.parse(html: "<p Class=One>One <p class=Two>Two <p CLASS=THREE>THREE").select("p")
        let one = els[0]
        let two = els[1]
        let thr = els[2]
    ...
    func testHasText() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p></div><div><p></p></div>")
    ...
    func testHtml() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p></div><div><p>There</p></div>")
        let divs = doc.select("div")
    ...
    func testSetHtml() {
        let doc = SwiftySoup.parse(html: "<p>One</p><p>Two</p><p>Three</p>")
        let ps = doc.select("p")
    ...
    func testVal() {
        let doc = SwiftySoup.parse(html: "<input value='one' /><textarea>two</textarea>")
    ...
    func testBefore() {
        let doc = SwiftySoup.parse(html: "<p>This <a>is</a> <a>jsoup</a>.</p>")
    ...
    func testEmpty() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello <b>there</b></p> <p>now!</p></div>")
    ...
    func testRemove() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello <b>there</b></p> jsoup <p>now!</p></div>")
        doc.outputSettings.prettyPrint = false
    ...
    func testParents() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p></div><p>There</p>")
    ...
    func testNot() {
        let doc = SwiftySoup.parse(html: "<div id=1><p>One</p></div> <div id=2><p><span>Two</span></p></div>")
    ...
    func testTagNameSet() {
        let doc = SwiftySoup.parse(html: "<p>Hello <i>there</i> <i>now</i></p>")
    ...
    func testTraverse() {
        let doc = SwiftySoup.parse(html: "<div><p>Hello</p></div><div>There</div>")
    ...
    func testForms() {
        let doc = SwiftySoup.parse(html: "<form id=1><input name=q></form><div /><form id=2><input name=f></form>")
        let els = doc.select("*")
    ...
    func testClassWithHyphen() {
        let doc = SwiftySoup.parse(html: "<p class='tab-nav'>Check</p>")
    ...
    func testSiblings() {
        let doc = SwiftySoup.parse(html: "<div><p>1<p>2<p>3<p>4<p>5<p>6</div><div><p>7<p>8<p>9<p>10<p>11<p>12</div>")
    ...
    func testEachText() {
        let doc = SwiftySoup.parse(html: "<div><p>1<p>2<p>3<p>4<p>5<p>6</div><div><p>7<p>8<p>9<p>10<p>11<p>12<p></p></div>")
    ...
    func testEachAttr() {
        let doc = SwiftySoup.parse(html: "<div><a href='/foo'>1</a><a href='http://example.com/bar'>2</a><a href=''>3</a><a>4</a>",
