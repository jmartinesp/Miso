...
class DocumentTypeTest: XCTestCase {
    ...
    private func htmlOutput(for html: String) -> String {
        let documentType = SwiftySoup.parse(html: html).childNodes.first!
        return documentType.outerHTML
    }
    
    private func xmlOutput(for xml: String) -> String {
        let documentType = SwiftySoup.parse(html: xml, baseUri: nil, parser: Parser.xmlParser).childNodes.first!
        return documentType.outerHTML
    }
