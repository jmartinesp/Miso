...
class AttributeParseText: XCTestCase {
    ...
    func testParsesRoughAttributeString() {
        ...
        let element = SwiftySoup.parse(html: html).elements(byTag: "a")[0]
    ...
    func testHandlesNewLinesAndReturns() {
        let html = "<a\r\nfoo='bar\r\nqux'\r\nbar\r\n=\r\ntwo>One</a>"
        let element = SwiftySoup.parse(html: html).elements(byTag: "a").first
    ...
    func testStrictAttributeUnescapes() {
        ...
        let elements = SwiftySoup.parse(html: html).elements(byTag: "a")
    ...
    func testDropsSlashFromAttributeName() {
        ...
        doc = SwiftySoup.parse(html: html, baseUri: nil, parser: Parser.xmlParser)
