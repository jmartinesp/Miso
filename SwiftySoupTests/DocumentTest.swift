...
class DocumentTest: XCTestCase {
    ...
    func testSetTextPreservesDocumentStructure() {
        let document = SwiftySoup.parse(bodyFragment: "<p>Hello</p>")
    ...
    func testTitles() {
        let noTitle = SwiftySoup.parse(bodyFragment: "<p>Hello</p>")
        let withTitle = SwiftySoup.parse(bodyFragment: "<title>First</title><title>Ignore</title><p>Hello</p>")
        ...
        let normalizeTitle = SwiftySoup.parse(bodyFragment: "<title>   Hello\nthere   \n   now   \n")
        XCTAssertEqual("Hello there now", normalizeTitle.title)
    }
    
    func testOutputEncoding() {
        let document = SwiftySoup.parse(bodyFragment: "<p title=π>π & < > </p>")
        // default is utf-8
    ...
    func testXhtmlReferences() {
        let document = SwiftySoup.parse(bodyFragment: "&lt; &gt; &amp; &quot; &apos; &times;")
        document.outputSettings.escapeMode = .xhtml
        XCTAssertEqual("&lt; &gt; &amp; \" ' ×", document.body?.html)
    }
    
    func testNormalizingStructure() {
        let document = SwiftySoup.parse(html: "<html><head><script>one</script><noscript><p>two</p></noscript></head><body><p>three</p></body><p>four</p></html>")
        XCTAssertEqual(
    ...
    func testHtmlAndXmlSyntax() {
        ...
        let document = SwiftySoup.parse(html: html)
    ...
    func htmlParseDefaultsToHtmlOutputSyntax() {
        let document = SwiftySoup.parse(bodyFragment: "x")
    ...
    func testHtmlAppendable() {
        ...
        let document = SwiftySoup.parse(html: htmlContent)
    ...
    func testDocumentsWithSameContentAreEqual() {
        let documentA = SwiftySoup.parse(html: "<div/>One")
        let documentB = SwiftySoup.parse(html: "<div/>One")
        let documentC = SwiftySoup.parse(html: "<div/>Two")
