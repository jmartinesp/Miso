...
class NodeTest: XCTestCase {
    ...
    func testSetBaseUriIsRecursive() {
        let doc = SwiftySoup.parse(html: "<div><p></p></div>")
    ...
    func testHandlesAbsPrefix() {
        let doc = SwiftySoup.parse(html: "<a href=\"/foo\">Hello</a>", baseUri: "https://google.com/")
    ...
    func testHandlesAbsOnImage() {
        let doc = SwiftySoup.parse(html: "<p><img src=\"/rez/osi_logo.png\" /></p>", baseUri: "https://google.com/")
    ...
    func testHandlesAbsPrefixOnHasAttr() {
        ...
        let doc = SwiftySoup.parse(html: "<a id=1 href='/foo'>One</a> <a id=2 href='https://google.com/'>Two</a>")
    ...
    func testLiteralAbsPrefix() {
        ...
        let doc = SwiftySoup.parse(html: "<a abs:href='odd'>One</a>")
    ...
    func testHandleAbsOnFileUris() {
        let doc = SwiftySoup.parse(html: "<a href='password'>One/a><a href='/var/log/messages'>Two</a>", baseUri: "file:/etc/")
    ...
    func testhHandleAbsOnLocalhostFileUris() {
        let doc = SwiftySoup.parse(html: "<a href='password'>One/a><a href='/var/log/messages'>Two</a>", baseUri: "file://localhost/etc/")
        let one = doc.select("a")[0]
    ...
    func testHandlesAbsOnProtocolessAbsoluteUris() {
        let doc1 = SwiftySoup.parse(html: "<a href='//example.net/foo'>One</a>", baseUri: "http://example.com/")
        let doc2 = SwiftySoup.parse(html: "<a href='//example.net/foo'>One</a>", baseUri: "https://example.com/")
        ...
        let doc3 = SwiftySoup.parse(html: "<img src=//www.google.com/images/errors/logo_sm.gif alt=Google>", baseUri: "https://google.com")
    ...
    func testAbsHandlesRelativeQuery() {
        let doc = SwiftySoup.parse(html: "<a href='?foo'>One</a> <a href='bar.html?foo'>Two</a>", baseUri: "https://jsoup.org/path/file?bar")
    ...
    func testAbsHandlesDotFromIndex() {
        let doc = SwiftySoup.parse(html: "<a href='./one/two.html'>One</a>", baseUri: "http://example.com")
        let a1 = doc.select("a").first
        XCTAssertEqual("http://example.com/one/two.html", a1?.absUrl(forAttributeKey: "href"))
    }
    
    func testRemove() {
        let doc = SwiftySoup.parse(html: "<p>One <span>two</span> three</p>")
    ...
    func testOwnerDocument() {
        let doc = SwiftySoup.parse(html: "<p>Hello")
        let p = doc.select("p").first
    ...
    func testBefore() {
        let doc = SwiftySoup.parse(html: "<p>One <b>two</b> three</p>")
    ...
    func testUnwrap() {
        let doc = SwiftySoup.parse(html: "<div>One <span>Two <b>Three</b></span> Four</div>")
    ...
    func testUnwrapNoChildren() {
        let doc = SwiftySoup.parse(html: "<div>One <span></span> Two</div>")
        let span = doc.select("span").first
        let node = span?.unwrap()
